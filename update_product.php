...
// Fetch product data - FIXED QUERY
// First, let's check what columns exist in your table
$product_query = $con->query("SELECT * FROM tbl_product WHERE pid = $id");
...
if (!$product) {
    ...
    $product_query = $con->query("SELECT * FROM tbl_product LIMIT 1");
    if ($product_query) {
        $sample = $product_query->fetch_assoc();
        echo "<script>alert('Product not found. Available columns: " . implode(', ', array_keys($sample)) . "');</script>";
    }
    ...
    <div class="flex">
        ...
        <div class="main-content w-full p-5">
            ...
            <div class="bg-white rounded-lg shadow p-6 mb-6">
                <h3 class="text-xl font-bold text-gray-800 mb-6 border-b pb-3">Update Product</h3>
                ...
                <form method="POST" enctype="multipart/form-data" class="max-w-3xl">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <input type="text" id="productName" name="txtpname" required
                                ...
                                value="<?= htmlspecialchars($product['pname']) ?>" placeholder="Enter product name">
                        </div>
                            ...
                            <select id="productCategory" name="txtcategory" required
                                ...
                                <?php foreach ($categories as $category): ?>
                                    <option value="<?= htmlspecialchars($category['cat_name']) ?>"
                                        <?= $category['cat_name'] == $product['category'] ? 'selected' : '' ?>>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-4">
                                <?php if (!empty($product['pic']) && file_exists($product['pic'])): ?>
                                    <img src="<?= htmlspecialchars($product['pic']) ?>" alt="Current product"
                                        class="current-image">
                                <?php else: ?>
                                    <div class="current-image flex items-center justify-center bg-gray-100">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="productDescription" name="txtpdesc" rows="4" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary transition"
                            placeholder="Enter product description"><?= htmlspecialchars($product['pdesc']) ?></textarea>
                    </div>
...
if (isset($_POST['btnsubmit'])) {
    ...
    // Handle file upload
    $pic = $product['pic']; // Keep existing image by default
    ...
    // Update database
    if (!empty($pname) && !empty($pdesc) && !empty($price) && !empty($category)) {
